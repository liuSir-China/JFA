...
public final class HelpText {
    ...
    public static String mainHelp() {
        return ""
                ...
                + "用法:\n"
                + "  jfa [--config <path>] [--format text|json|both] [--verbose] [--quiet] <command> [options]\n\n"
                + "主入口:\n"
                ...
                + "诊断行为:\n"
                + "  diagnose/analyze 默认向 stderr 打印 [JFA] 分步进度（解析目标、准备运行目录、日志倒查、jstat 采样、hprof 复用/采集、线程分析、compare 等待、写报告）。\n"
                + "  --quiet 只关闭这些中途步骤；结束后仍打印报告绝对路径（报告文件 / JSON 报告）。\n"
                + "  --verbose 与默认进度兼容，可附加更细的路径/体积信息；主步骤不需要 --verbose。\n"
                + "  --format json 时 stdout 仍是纯 JSON，进度在 stderr。\n"
